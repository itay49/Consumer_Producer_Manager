...
{
    public class EmsSender : IProducer
    {
        ...
        public EmsSender(DetailsConnectEMS detailsEMS, string clientID,
        ...
        public bool Send(string data, string applicationProperties)
        {
            ...
            {
                _logger.LogError($"[EmsSender:Send] Exception occurred while writing message:{data} to ems to queue:{m_Dest}. {ex}");
                retVal = false;
            }
            return retVal;
        }
        public bool Send(string message, string destination, string applicationProperties)
        {
            ...
            {
                _logger.LogError($"[EmsSender:Send] Exception occurred while writing message:{message} to ems to queue:{destination}. {ex}");
                retVal = false;
            }
            return retVal;
        }
        public bool Send(string message, string dest, string detailsHeaders, string applicationProperties)
        {
            ...
            {
                _logger.LogError($"[EmsSender:Send] ParserException before sending message:{message} to:{dest} with headers:{detailsHeaders}. . {ex}");
                retVal = false;
            }
            catch (Exception ex)
            {
                _logger.LogError($"[EmsSender:Send] Exception occurred while writing message:{message} to ems to queue:{dest}. {ex}");
                retVal = false;
            }
            return retVal;
        }
