...
{
    public class RabbitListener : IConsumer
    {
        public string Name { get; set; }
        private RabbitMQManager rabbitMQMgr;
        private DetailsConnectRabbit _detailsRabbit;
        private static ILogger _logger;
        public RabbitListener(DetailsConnectRabbit detailsListenerRabbit,
        ...
        public bool Start()
        {
            ...
            {
                _logger.LogError($"[RabbitListener:Start] error on Starting Consumer. {ex}");
                return false;
            }
        }
        ...
        public bool Stop()
        {
            ...
            {
                _logger.LogError($"[RabbitListener:Stop] error on stoping Consumer. {ex}");
                return false;
            }
        }
        ...
        public bool Acknowledge(string messageID)
        {
            ...
            {
                _logger.LogError($"[RabbitListener:Acknowledge] error on Acknowledge. {ex}");
                return false;
            }
        }
        ...
        public bool Commit(string messageID)
        {
            ...
            {
                _logger.LogError($"[RabbitListener:Commit] error on Commit. {ex}");
                return false;
            }
        }
        ...
        public bool Rollback(string messageID)
        {
            ...
            {
                _logger.LogError($"[RabbitListener:Rollback] error on Rollback. {ex}");
                return false;
            }
        }
        ...
        public bool Init()
        {
            ...
            {
                _logger.LogError($"[RabbitListener:Init] error on Init Consumer. {ex}");
                return false;
            }
        ...
        public void StartConsumer(string destName, Action<object, EventArgs> action, int index = 0)
        {
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if(sessionConsumer == null || sessionConsumer.Consumer == null)
                        {
                            ex= $"[RabbitListener:StartConsumer] try to consume to queue:{destName} to session number:{index}, but it is not exist in our queueConsumers. Your destName is probably wrong. Also check that your index <= to Count in DetailsConnectRabbit";
                        }
                        else
                        {
                            ex = $"[RabbitListener:StartConsumer] try to consume to queue:{destName} to session number:{index}, but session is already consumed! Your index is probably wrong";
                        }
                        _logger.LogError(ex);
            ...
            {
                _logger.LogError($"[RabbitListener:StartConsumer]Error on StartConsumer. {ex}");
                throw;
            }
        }
    }
