...
{
    public class FileListener : IConsumer
    {
        public string Name { get; set; }
        private event EventHandler OnMessage;
        private Timer timer;
        private int time;
        private string destinationFrom;
        private static ILogger _logger;

        public FileListener(DetailsConnectFile detailsFile, string name, ILogger log)
        {
            _logger = log;
            this.Name = name;
        ...
        public bool Acknowledge(string messageID)
        {
            ...
            {
                _logger.LogError($"[FileListener:Acknowledge] error on Acknowledge messege:{messageID}. {ex}");
                return false;
            }
        }
        ...
        {
            _logger.LogError($"[FileListener:Commit] error on Commit messege:{messageID}. Commit not implement");
            return false;
        }
        public bool Rollback(string messageID)
        {
            _logger.LogError($"[FileListener:Rollback] error on Rollback messege:{messageID}. Rollback not implement");
            return false;
        }
        ...
        public bool Start()
        {
            ...
            {
                _logger.LogError($"[FileListener:Start] error on starting Consumer. {ex}");
                return false;
            }
        }
        public bool Stop()
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"[FileListener:Stop] error on stoping Consumer. {ex}");
                return false;
            }
        }
        ...
        public bool Init()
        {
            _logger.LogError($"[FileListener:Init] Init not implement");
            return false;
        }

        public bool CreateTemporaryDestination(string destination)
        {
            _logger.LogError($"[FileListener:CreateTemporaryDestination] CreateTemporaryDestination not implement");
            return false;
        }

        public bool DestroyTemporaryDestination(string destination)
        {
            _logger.LogError($"[FileListener:DestroyTemporaryDestination] DestroyTemporaryDestination not implement");
            return false;
        }
        public void Dispose()
        {
